...
int main(void){
	...
	cipher_1 = encrypt();
	cycles = TIMER0 - cycles;
	printf("encipher 1: %8x%8x, %d cycles\n", cipher_1[0], cipher_1[1], cycles);

	cycles = TIMER0;
	cipher_2 = encrypt();
	cycles = TIMER0 - cycles;
	printf("encipher 2: %8x%8x, %d cycles\n", cipher_2[0], cipher_2[1], cycles);

	cycles = TIMER0;
	cipher_3 = encrypt();
	cycles = TIMER0 - cycles;
	printf("encipher 3: %8x%8x, %d cycles\n", cipher_3[0], cipher_3[1], cycles);
	...
	decrypt(cipher_1);
	cycles = TIMER0 - cycles;
	printf("decipher 1: %8x%8x, %d cycles\n", msg[0], msg[1], cycles);

	cycles = TIMER0;
	decrypt(cipher_2);
	cycles = TIMER0 - cycles;
	printf("decipher 2: %8x%8x, %d cycles\n", msg[0], msg[1], cycles);

	cycles = TIMER0;
	decrypt(cipher_3);
	cycles = TIMER0 - cycles;
	printf("decipher 3: %8x%8x, %d cycles\n", msg[0], msg[1], cycles);
